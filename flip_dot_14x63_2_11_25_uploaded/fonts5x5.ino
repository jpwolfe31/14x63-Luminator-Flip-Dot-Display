...
const static byte font_data_5x5[] = {
  // ascii 0x30 is the 0
  // need to subtract this from the lookup value since 0x00-0x2f not included here
  0b10101,
  0b01010,
  0b10101,
  0b01010,
...
void get_char_fd_array_5x5(char char_out)
{
byte char_base_addr;
char_base_addr = byte(char_out);
//columns first, then rows unlike 7x5 font - 7x5 and 5x5 fonts presented differently
  for (int r = 0; r <= 4; r++){
    // (4-c) to feed in other direction from 7x5 font
    for (int c = 0; c <=4; c++){ 
      fd_array_5x5[c][r] = ((font_data_5x5[(char_base_addr * 5) + r]) >> (4-c)) & 0x01; //bitwise and
      }  
  }
}


void displayCharacter_5x5(int charNum, char charOut)
{
// for 5x5 font
int dotInput;
get_char_fd_array_5x5(charOut); // populates fd_array 5x5 with charOut
// display font
for (int columnInput = 0; columnInput <= 4; columnInput++){ // font is 5 by 5
  for (int rowInput = 0; rowInput <= 4; rowInput++){
    dotInput = fd_array_5x5[columnInput][rowInput]; // get dot value
    // 1 space between chars.  Also start at column 2 and row 5
    setCell(columnInput + 2 + (charNum * (5+1)), (rowInput + 5), dotInput); // adjust columns by charNum
    }  
  }
return;
}
